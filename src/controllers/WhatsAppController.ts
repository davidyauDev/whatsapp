...
class WhatsAppController {
  ...
  sendQr(req: Request, res: Response, io: Server) {
    try {
      ...
      fs.createReadStream(`${process.cwd()}/QR/${sessionId}-qrcode.svg`).pipe(
        res
      );
    } catch (ex: any) {
  ...
  try {
    ...
    const client = this.ws.getClientSessionId(sessionId);
    console.log("client:", client);
    if (!client || !client.info) throw new Error("No has iniciado session");

    const files: Express.Multer.File[] = (req.files as any) || [];
    console.log("[Controller] files.length:", files.length, "message?:", !!message);

    if (files.length > 0) {
      // PRIMER archivo con caption
      await this.sms.sendMedia(client, newNumber, files[0].path, sessionId, message);

      // Si hay más, sin caption
      for (let i = 1; i < files.length; i++) {
        await this.sms.sendMedia(client, newNumber, files[i].path, sessionId);
      }

      // *** NO MANDAR TEXTO AQUÍ ***
      console.log("[Controller] NO se envía texto adicional");
    } else {
  ...
  async clientDestroy(req: Request, res: Response) {
    try {
      ...
      if (client != undefined) {
        ...
        res
          .status(409)
          .send({ success: false, message: "DISPOSITIVO NO INICIALIZADO" });
      }
    } catch (error: any) {
      res.status(500).send({ success: false, message: error.message });
    }
  ...
  async clientLogout(req: Request, res: Response) {
    try {
      ...
      if (client != undefined) {
        ...
        res
          .status(409)
          .send({ success: true, message: "DISPOSITIVO NO INICIALIZADO" });
      }
    } catch (error) {
      res.json({
        success: false,
